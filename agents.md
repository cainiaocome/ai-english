...
│       └── ci.yml
├── .cargo/
│   └── config.toml       # Configures target-dir = "src-tauri/target"
...
## 16. Code Signing Notes (Important for Distribution)

For the app to run on macOS without showing "corrupted" errors:

### Development builds (local testing)
- Use ad-hoc signing: `codesign --force --deep --sign - path/to/App.app`
- Or remove quarantine: `xattr -cr path/to/App.app`

### CI builds
- Configure Tauri to skip signing for CI artifacts
- Users downloading CI builds must run: `xattr -cr /path/to/ai-english.app`

### Production distribution
- Requires Apple Developer account
- Must be code signed and notarized
- Configure in `tauri.conf.json` with proper signing identity
